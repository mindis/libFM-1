...
class Data {
	...
	public:	
		...
		void loadFromStdin(istream* input);
...
private:
	void loadDataFromDistinctFiles(int load_from, const std::string& filename);

	void loadDataMetaInfo(const std::string& filename);
	void loadDataMetaInfo(istream* input);
	void initDataParameters(int numRows, uint64 numValues);
	void loadData(istream* input);
};

void Data::loadFromStdin(istream* input)
{
	loadDataMetaInfo(input);

	std::cout << "Waiting for data from stdin" << std::endl;
	loadData(input);
}
...
void Data::loadFromFile(const std::string& filename) {
	...
	if (load_from > 0) {
		loadDataFromDistinctFiles(load_from, filename);
	}
	else {
		loadDataMetaInfo(filename.c_str());
...
void Data::initDataParameters(int numRows, uint64 numValues)
{
	LargeSparseMatrixMemory<float>* data = new LargeSparseMatrixMemory<DATA_FLOAT>();
	this->data = data;

	data->data.setSize(numRows);
	target.setSize(numRows);
	data->num_cols = num_feature;
	data->num_values = numValues;
	MemoryLog::getInstance().logNew("data_float", sizeof(sparse_entry<DATA_FLOAT>), numValues);
}

void Data::loadDataMetaInfo(istream* input)
{
	min_target = 0;
	max_target = 0;
	num_feature = 0;
	...
	std::cout << "Waiting for next values: " << std::endl
			  ...
			  << "features count, size = " 		 << sizeof(num_feature) << " byte(s)," << std::endl
	...
	std::cout << "min target = "  << min_target  << std::endl
			  << "max_target = "  << max_target  << std::endl
			  << "num_feature = " << num_feature << std::endl
			  << "num_rows = " 	  << num_rows 	 << std::endl
			  << "num_values = "  << num_values  << std::endl;
	initDataParameters(num_rows, num_values);
}

void Data::loadDataMetaInfo(const std::string& filename)
{
	...
	max_target = -std::numeric_limits<DATA_FLOAT>::max();
	{
		...
		while (!fData.eof()) {
			...
			if (sscanf(pline, "%f%n", &_value, &nchar) >= 1) {
				...
				if ((*pline != 0) && (*pline != '#')) {
					throw "cannot parse line \"" + line + "\" at character "
							+ pline[0];
				}
			} else {
				throw "cannot parse line \"" + line + "\" at character "
	...
	std::cout << "num_rows=" << num_rows << "\tnum_values=" << num_values
			<< "\tnum_features=" << num_feature << "\tmin_target=" << min_target
			<< "\tmax_target=" << max_target << std::endl;
	initDataParameters(num_rows, num_values);
}

void Data::loadData(istream* input)
{
	DVector<sparse_row<DATA_FLOAT> >& data 		 = ((LargeSparseMatrixMemory<DATA_FLOAT>*) (this->data))->data;
	int								  num_rows   = this->data->getNumRows();
	uint64 							  num_values = this->data->getNumValues();
	sparse_entry<DATA_FLOAT>* 		  cache 	 = new sparse_entry<DATA_FLOAT> [num_values];
	
	// (2) read the data
	{
		volatile int row_id = 0;
		uint64 cache_id = 0;
		DATA_FLOAT _value;
		int nchar, _feature;
...
void Data::loadDataFromDistinctFiles(int load_from, const std::string& filename) {
	uint num_values = 0;
	...
	if (has_x) {
		std::cout << "data... ";
		if (load_from == 1) {
			this->data = new LargeSparseMatrixHD<DATA_FLOAT>(filename + ".data",
					this_cs);
		} else {
			this->data = new LargeSparseMatrixHD<DATA_FLOAT>(filename + ".x",
					this_cs);
		}
	...
	if (has_xt) {
		...
		if (load_from == 1) {
			this->data_t = new LargeSparseMatrixHD<DATA_FLOAT>(
					filename + ".datat", this_cs);
		} else {
			this->data_t = new LargeSparseMatrixHD<DATA_FLOAT>(filename + ".xt",
					this_cs);
		}
	...
	num_cases = target.dim;
	std::cout << "num_cases=" << this->num_cases << "\tnum_values="
			<< num_values << "\tnum_features=" << this->num_feature
