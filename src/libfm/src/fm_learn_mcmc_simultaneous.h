...
class fm_learn_mcmc_simultaneous : public fm_learn_mcmc {
	protected:
		...
		virtual void _learn(Data& train) {
			
			uint num_complete_iter = 0;

			// make a collection of datasets that are predicted jointly
			DVector<Data*> main_data;
			DVector<e_q_term*> main_cache;
			if (test != NULL) {
				main_data.setSize(2);
				main_cache.setSize(2);
				main_data(0) = &train;
				main_data(1) = test;
				main_cache(0) = cache;
			...
			for (uint i = num_complete_iter; i < num_iter; i++) {
				...
				if (task == TASK_REGRESSION) {
					...
					std::cout << "#Iter=" << std::setw(3) << i << "\tTrain=" << acc_train;
					if (test != NULL) {
						...
						std::cout << "\tTest=" << acc_test_all << "\tTest(ll)=" << ll_test_all;
