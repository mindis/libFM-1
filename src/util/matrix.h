...
template <typename T> class DMatrix {
	public:
		...
		T& operator() (unsigned x, unsigned y) {
   		...
   		void saveToStream(ostream* out, bool one_row, bool has_header = false) {
   			if (has_header) {
   		...
   		void saveToFile(std::string filename, bool one_row, bool has_header = false) {
			...
			if (out_file.is_open())	{
				saveToStream(&out_file, one_row, has_header);
				out_file.close();
			} else {
		...
		void saveToBinaryStream(ostream* out) {
